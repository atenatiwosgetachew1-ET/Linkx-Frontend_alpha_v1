...
function Windows({ id, type, sessionId, loadscreenState, isSideBarMenuOpen, action, selectedContent, selectedSubContent, formIResponse,formIIResponse,formToolResponse,batchFilesSearchResults,searchResultsVisible,searchPlaceholder,batchFilesCollection, batchFilesDataframeInfoI, batchFilesDataframeInfoII, sourceSessionLog, onClose, onMove, zIndex, onFocus }) {
                    ...
                    <div className="batch_connection_form_container">
                                ...
                                {batchFilesDataframeInfoI && batchFilesDataframeInfoI.length > 0 && (
                                  <table id="batch_files_dataframe_infos_left_table" cellPadding='0'>
                                    <tbody>
                                      ...
                                      <tr>
                                        <td>Total columns</td>
                                        <td>{batchFilesDataframeInfoI[2]}</td>
                                      </tr>
                                    </tbody>
                                  </table>
                                )}
                                ...
                                {batchFilesDataframeInfoI && batchFilesDataframeInfoI.length > 0 && (
                                  <table id="batch_files_dataframe_infos_right_table" cellPadding='0'>
                                    <tbody>
                                      <tr>
                                        <td>Broker/API</td>
                                        <td>{batchFilesDataframeInfoI[0]}</td>
                                      </tr>
                                      <tr>
                                        <td>Storage</td>
                                      ...
                                      <tr>
                                        <td>Tool</td>
                                        <td>{batchFilesDataframeInfoI[5]}</td>
                                      </tr>
                                    </tbody>
                                  </table>
                                )}
                      ...
                      {selectedSubContent === "batch_input_form_pageIV" && (
                        <div>
                          <div id="batch_input_form_response" className="form_response_container"
                            style={{
                              color: formIResponse === "Session running..." ? '#009' : formIResponse === "Connecting..." ? '#999' : formIResponse === "Connection established!" ? '#090' : formIResponse === "Connection failed!" ? '#900' : '',
                              backgroundColor: formIResponse === "Session running..." ? 'rgba(0, 0, 255, 0.2)' : formIResponse === "Connecting..." ? 'rgba(0,0,0,0.1)' : formIResponse === "Connection established!" ? 'rgba(0, 255, 0, 0.2)' : formIResponse === "Connection failed!" ? 'rgba(255, 0, 0, 0.2)' : ''
                            }}>
                            <span>
                              <Icons
                                id="window_live_source_option"
                                type={
                                  formIResponse === "Connecting..." ? "loadingx" :
                                  formIResponse === "Session running..." ? "loadingx" :
                                  formIResponse === "Connection established!" ? "correctx" : "errorx"
                                }
                                condition="True"
                              />
                          ...
                          <form className="batch_files_dataframe_form" onSubmit={(e) => { e.preventDefault()}}>
                            ...
                            <fieldset className="batch_files_dataframe_filter">
                              ...
                              <textarea readOnly style={{width:'97%',height:'95%',backgroundColor:'rgba(255,255,255,1)',border:'0.2vh solid #EEE',color:'#AAA'}} value={sourceSessionLog}></textarea>
                            </fieldset>
                          </form>
                        </div>
                      )}
                    ...
                    <div className="batch_connection_form_pager_container">
                      ...
                      <button onClick={() => {
                        ...
                        disabled={
                          ...
                          batchFilesCollection.length> 0  ||
                          selectedSubContent === "batch_input_form_pageIII" && 
                          batchFilesDataframeInfoI.length> 0  ? '': 'True' 
                        }>Next</button>
                    </div>
                  </div>
                )}
...
function Root() {
  ...
  const [sourceSessionLog, setSourceSessionLog] = useState(null);
  ...
  const generateSessionId = (id) => {
    let max=1000000;
    let min=0;
    let sessionId= Math.floor(Math.random() * (max - min + 1 + id)) + min;
  ...
  const handleCreateWindows = (type) => {
    ...
    if (type==="source"){
      const session = generateSessionId(id);
      setSessionId(session);
      const maxZ = Math.max(0, zIndexCounter);
      setWindows(prev => [...prev, { id:id, type, zIndex: maxZ + 1, sessionId:session, selectedContent: null, selectedSubContent: null, formData: {}, formIResponse: null, formToolResponse: null, batchFilesSearchResults:null}]);
      setZIndexCounter(prev => prev + 1);
    }
    else{
      setWindows(prev => [...prev, { id, type, zIndex: zIndexCounter, sessionId, selectedContent: null, selectedSubContent: null, formData: {}, formIResponse: null, formToolResponse: null, batchFilesSearchResults:null}]);
      setZIndexCounter(prev => prev + 1);
    }
  };
      ...
      prev.map(w => {
        ...
        if (menuId === "batch_input_form_swap" && action === "page_III") {
          ...
          debounceRef.current = setTimeout(() => {
            setWindows(prev =>
              prev.map(w =>
               w.id === id ? { ...w, batchFilesDataframeInfoI:[],loadscreenState: true } : w
              )
            );
            //Requesting dataframe creation
            const payload = {
            ...
            .then((data) => {    
                ...
                if (data.message == "success!"){
                  ...
                  setWindows(prev =>
                    prev.map(w =>
                      w.id === id ? { ...w, batchFilesDataframeInfoI:arrayData,loadscreenState: false,selectedContent:newContent,selectedSubContent:newSubContent } : w
                    )
                  );
                }
                else {
                  alert("faild to create a dataframe")
                  setWindows(prev =>
                    prev.map(w =>
        ...
        if (menuId === "batch_input_form_swap" && action === "page_IV") {
          ...
          debounceRef.current = setTimeout(() => {
            setWindows(prev =>
              prev.map(w =>
               w.id === id ? { ...w, sourceSessionLog:null,loadscreenState: true } : w
              )
            );
            ...
            const payload = {
                id: "start_session",
                value:{"window_id":id,"session_id":sessionId}// Add filter request here
              };
            fetch("http://localhost:5000/live_batch_files", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
            ...
            .then((data) => { 
              ...
              setWindows(prev =>
                    prev.map(w =>
                      w.id === id ? { ...w, loadscreenState: false} : w
                    )
                  );  
                // var arrayData=Object.values(data.results)
                // setBatchFilesDataframeInfoI(arrayData)
